...
- OpenAI Responses API endpoint: `/v1/responses` for clients that use the Responses API protocol (OpenAI Codex CLI, etc.). Supports both streaming and non-streaming modes with function call tools.
...
- FastAPI exposes Anthropic-compatible routes (`/v1/messages`, `/v1/messages/count_tokens`, `/v1/models`), OpenAI-compatible routes (`/v1/chat/completions`, `/v1/completions`), and OpenAI Responses API routes (`/v1/responses`).
